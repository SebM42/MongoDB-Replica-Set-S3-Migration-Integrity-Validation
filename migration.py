...
def insert_records_to_collection(db, collection_name, records):
    coll = db[collection_name]
    try:
        res = coll.insert_many(records)
        return {"inserted_count": len(res.inserted_ids), "error": None}
    except BulkWriteError as bwe:
        return {"inserted_count": bwe.details.get("nInserted", 0), "error": str(bwe.details)}
...
def process_file(s3, bucket, key, db):
    ...
    # Insert into Mongo
    insert_res = insert_records_to_collection(db, collection_name, records)
